...
import (
	...
	"github.com/markoxley/mxmsorm/config"
	"github.com/markoxley/mxmsorm/utils"
...
const (
	//connectionPattern = "server=" + *server + ";user id=" + *userid + ";password=" + *password + ";database=" + *database
	// connectionPattern = "server=%s;database=%s;user id=%s;password=%s;port=%d"
	//  "sqlserver://"+dbuser+":"+dbpassword+"@"+dbname+":1433?database="+database
	connectionPattern = "sqlserver://%s:%s@%s:%d?database=%s"
)
...
func connect() (*sql.DB, error) {
	cs := fmt.Sprintf(connectionPattern, conf.User, conf.Password, conf.Host, conf.Port, conf.Name)
	tdb, err := sql.Open("mssql", cs)
	if err != nil {
		return nil, err
	}
	return tdb, nil
